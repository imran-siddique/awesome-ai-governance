...
- [PIC Standard (Provenance & Intent Contracts)](https://github.com/madeinplutofabio/pic-standard) - Open, local-first protocol for pre-execution action gating in AI agents: before a high-impact tool call runs, the agent declares intent, impact, provenance, evidence, and the target tool, and the verifier returns allow or block, failing closed when required evidence is missing or invalid. Supports sandboxed SHA-256 file evidence, Ed25519 signature evidence, trusted keyrings with expiry and revocation, strict trust sanitization (self-asserted trust is downgraded to untrusted), and opt-in canonical attestation-object signing. Canonicalization (PIC-CJSON/1.0) builds on RFC 8785 (JCS); PIC-Core / PIC-Evidence / PIC-Full conformance profiles run a portable vector suite in CI. Software-layer and hardware-free. Apache-2.0. Drafting toward PIC/1.0.
